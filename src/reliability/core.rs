...
pub(crate) fn surv_to_cumhaz(surv: f64) -> f64 {
    if surv <= 0.0 {
        f64::INFINITY
    } else {
        -surv.ln()
...
pub(crate) fn cumhaz_to_surv(cumhaz: f64) -> f64 {
    (-cumhaz).exp()
}

pub(crate) fn surv_to_cloglog(surv: f64) -> f64 {
    if surv <= 0.0 {
        f64::INFINITY
    } else if surv >= 1.0 {
        f64::NEG_INFINITY
...
pub(crate) fn cloglog_to_surv(cloglog: f64) -> f64 {
    (-cloglog.exp()).exp()
}

pub(crate) fn surv_to_loglogistic(surv: f64) -> f64 {
    if surv <= 0.0 {
        f64::INFINITY
    } else if surv >= 1.0 {
        f64::NEG_INFINITY
...
pub(crate) fn loglogistic_to_surv(logit: f64) -> f64 {
    1.0 / (1.0 + logit.exp())
}
        ...
        for i in 0..surv.len() {
            ...
            let (new_se, l, u) = match scale_enum {
                ...
                ReliabilityScale::Probit => {
                    if s > 0.0 && s < 1.0 && se_val > 0.0 {
                        let p = 1.0 - s;
                        let z_val = normal_inverse_cdf(p);
                        let phi =
